...
async function loadWebAdmin() {
  try {
    ...
    if (portTaken) {
      console.log(`【风云AI-网页管理端】检测到端口 ${port} 已被占用，服务已在另一处启动`);
      console.log(`【风云AI-网页管理端】停止当前服务启动，任凭另一处继续运行`);
      isAnotherInstanceRunning = true;
      webAdminLoaded = true; // 标记为已加载，因为服务已经在运行
      console.log(`【风云AI-网页管理端】✅ 服务已在另一处运行 (端口: ${port})`);
      
      // 获取外网IP并显示地址
      const outerIp = await getOuterIp();
      console.log(`【风云AI-网页管理端】管理端地址：http://${outerIp}:${port}`);
      console.log(`【风云AI-网页管理端】本地地址：http://127.0.0.1:${port}`);
      
      return true;
    }
    
    console.log(`【风云AI-网页管理端】端口 ${port} 可用，开始启动服务`);
    
    try {
      // 尝试动态导入web-admin模块
      console.log(`【风云AI-网页管理端】正在导入模块...`);
      const module = await import('file://' + webAdminModulePath);
      webAdminModule = module;
      
      // 检查模块是否有默认导出或特定导出
      if (module.default) {
        console.log(`【风云AI-网页管理端】模块有默认导出`);
      }
      
      if (module.setSecurityConfig) {
        console.log(`【风云AI-网页管理端】模块有setSecurityConfig方法`);
      }
      
      // 将安全配置传递给web-admin
      if (module.setSecurityConfig) {
        module.setSecurityConfig(SECURITY_CONFIG);
        console.log(`【风云AI-网页管理端】已传递安全配置`);
      }
      
      // 尝试调用初始化方法（如果有）
      if (module.init && typeof module.init === 'function') {
        console.log(`【风云AI-网页管理端】调用init方法`);
        await module.init();
      }
      
      webAdminLoaded = true;
      isAnotherInstanceRunning = false;
      console.log(`【风云AI-网页管理端】✅ 服务启动成功`);
      
      // 获取外网IP
      const outerIp = await getOuterIp();
      console.log(`【风云AI-网页管理端】外网IP: ${outerIp}`);
      console.log(`【风云AI-网页管理端】管理端地址：http://${outerIp}:${port}`);
      console.log(`【风云AI-网页管理端】本地地址：http://127.0.0.1:${port}`);
...
export class XRKAIAssistant extends plugin {
  ...
  async init() {
    ...
    if (isAnotherInstanceRunning) {
      console.log(`├─ ⚓ 网页管理端: ✅ 已由另一处运行 (端口: ${SECURITY_CONFIG.webAdminPort})`);
    } else {
      console.log(`├─ ⚓ 网页管理端: ${webAdminLoaded ? '✅ 已加载' : '❌ 未加载'}`);
    }
    
    if (webAdminLoaded) {
      console.log(`├─ 📡 管理端口: ${SECURITY_CONFIG.webAdminPort || '未设置'}`);
    }
